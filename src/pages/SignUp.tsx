    ...
    try {
      ...
      const { error: profileError } = await supabase
        ...
        .upsert({
          ...
          time_of_birth: profileData.timeOfBirth,
          timezone: profileData.timezone,
          place_of_birth: profileData.placeOfBirth,
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div className="space-y-4">
              ...
              <TimePicker
                use12Hours
                format="h:mm A"
                className="w-full h-12 rounded-full border border-rose-200 bg-white/80 focus:outline-none focus:ring-2 focus:ring-rose-200 focus:border-transparent"
                value={dayjs(profileData.timeOfBirth, 'HH:mm')}
                onChange={(time) => {
                  if (time) {
                    setProfileData(prev => ({
                      ...prev,
                      timeOfBirth: time.format('HH:mm')
                    }));
                  }
                }}
                popupClassName="rounded-xl border border-rose-200 shadow-lg"
                minuteStep={1}
                showNow={false}
                allowClear={false}
              />
